...
# Additional sensitive points beyond the 10 traditional planets.
# Chiron — wounded healer asteroid.
# NorthNode — True Node (mean alternative: swe.MEAN_NODE). SouthNode is derived (NorthNode + 180°).
# Lilith — Mean apogee of the Moon, a.k.a. Black Moon Lilith.
POINTS = {
    "Chiron": swe.CHIRON,
...
def calc_points(jd_ut: float) -> Dict[str, Dict[str, Any]]:
    results: Dict[str, Dict[str, Any]] = {}
    for name, body_id in POINTS.items():
        xx, _retflags = swe.calc_ut(jd_ut, body_id, DEFAULT_FLAGS)
        longitude = xx[0]
        speed_longitude = xx[3]
        results[name] = build_position(longitude, speed_longitude)
    # Derive South Node as the geometric opposite of North Node.
    if "NorthNode" in results:
        nn = results["NorthNode"]
        south_lon = (nn["longitude"] + 180.0) % 360.0
        results["SouthNode"] = build_position(south_lon, -nn["speed_longitude"])
    return results
...
def build_chart(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Default to looking for ephemeris files alongside this script (where seas_18.se1
    # is stored for Chiron). Override with SE_EPHE_PATH if you keep them elsewhere.
    ephe_path = os.environ.get("SE_EPHE_PATH") or str(Path(__file__).resolve().parent)
    swe.set_ephe_path(ephe_path)

    planet_positions = calc_planets(jd_ut)
    points = calc_points(jd_ut)
    ...
    point_house_placement = assign_houses(points, houses)

    aspect_objects: Dict[str, Dict[str, Any]] = {}
    aspect_objects.update(planet_positions)
    aspect_objects.update(points)
    ...
    return {
        ...
        "point_house_placement": point_house_placement,
        "aspects": aspects,
    }
